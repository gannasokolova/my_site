...
@if ($paginator->lastPage() > 1)
    <ul class="pagination pagination_articles">
        ...
        @for ($i = 1; $i <= $paginator->lastPage(); $i++)
            <?php
            $half_total_links = floor($link_limit / 2); //(7/2) = 3
            $from = $paginator->currentPage() - $half_total_links; //11-3=8
            $to = $paginator->currentPage() + $half_total_links; // 11+3=14
            if ($paginator->currentPage() < $half_total_links) { //2<3
